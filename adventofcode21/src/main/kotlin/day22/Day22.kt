...
fun main() {
    ...
    val c1 = Cuboid(0..10, 0..10, 0..10, true)
    val c2 = Cuboid(5..15, 5..15, 5..15, true)
    val c3 = Cuboid(7..11, 7..9, 7..9, true)
    val c4 = Cuboid(17..20, 17..29, 17..29, true)
    val i1 = c1.intersect(c2)
    val i2 = i1.intersect(c3)

    val i3 = c1.intersect(c4)
    println(i3)
...
//    Day22().calc(small)
//    Day22().calc(test)
//    Day22().calc(rebootSteps)
    Day22().calc2(rebootSteps)
//    Day22().calc2(realData)
...
class Day22 {
    ...
    fun calc(data: Array<String>) {
        val reactor = Reactor()
        data.forEach { line ->
            val cuboid = createCuboid(line, true)
            if (cuboid != null) reactor.activate(cuboid)
        }
        println(reactor.countActive())
    }

    private fun createCuboid(line: String, clean: Boolean): Cuboid? {
        val split = line.split(",")
        var on = false
        val x = if (line.startsWith("on")) {
            on = true
            split[0].replace("on x=", "").split("..")
        } else {
            split[0].replace("off x=", "").split("..")
        }
        val y = split[1].replace("y=", "").split("..")
        val z = split[2].replace("z=", "").split("..")


        val xRange = x[0].toInt()..x[1].toInt()
        val yRange = y[0].toInt()..y[1].toInt()
        val zRange = z[0].toInt()..z[1].toInt()

        if (clean) {
            if (checkOverlap(xRange, yRange, zRange)) {
                val cuboid = Cuboid(xRange, yRange, zRange, on)
                println(cuboid)

                cuboid.clean()
                return cuboid
            }
        } else return Cuboid(xRange, yRange, zRange, on)
        return null
    }

    private fun checkOverlap(xRange: IntRange, yRange: IntRange, zRange: IntRange): Boolean {
        return isOverlapping1D(xRange) && isOverlapping1D(yRange) && isOverlapping1D(zRange)
    }

    private fun isOverlapping1D(range: IntRange): Boolean {
        val reactorBox = -50..50
        return range.first >= reactorBox.first && range.last <= reactorBox.last
    }

    fun calc2(data: Array<String>) {
        val cuboids = data.map { line ->
            createCuboid(line, false)
        }
//        var xRange = 0..0
//        var yRange = 0..0
//        var zRange = 0..0
...
//            if (cuboid != null) {
//                if (cuboid.x.first < xRange.first) {
//                    xRange = cuboid.x.first..xRange.last
//                }
//                if (cuboid.x.last > xRange.last) {
//                    xRange = xRange.first..cuboid.x.last
//                }
//
//                if (cuboid.y.first < yRange.first) {
//                    yRange = cuboid.y.first..yRange.last
//                }
//                if (cuboid.y.last > yRange.last) {
//                    yRange = yRange.first..cuboid.y.last
//                }
//
//                if (cuboid.z.first < zRange.first) {
//                    zRange = cuboid.z.first..zRange.last
//                }
//                if (cuboid.z.last > zRange.last) {
//                    zRange = zRange.first..cuboid.z.last
//                }
//            }
//        }
//        val field = Cuboid(xRange, yRange, zRange, false)
            ...
            if (cuboid != null) {
                if (list.isEmpty()) {
                    ...
                    if (cuboid.on) {
                        ...
                        var localSum: Long = 0
                        list.forEach { c ->
                            val curSize = cuboid.size()
                            val size2 = c.intersect(cuboid).size()
                            val diff = curSize - size2
                            localSum += diff
                        }
...
class Reactor {
    private val values = Array(101) { Array(101) { BooleanArray(101) } }
    fun activate(cuboid: Cuboid) {
        val on = cuboid.on

        for (x in cuboid.x) {
            for (y in cuboid.y) {
                for (z in cuboid.z) {
                    val pos = arrayOf(x + 50, y + 50, z + 50)
                    values[pos[0]][pos[1]][pos[2]] = on
                }
            }
        }
    }
    ...
    fun countActive(): Int {
        ...
        for (x in values.indices) {
            for (y in 0 until values[x].size) {
                for (z in 0 until values[x][y].size) {
                    if (values[x][y][z]) {
                        sum++
                    }
                }
...
class Cuboid(var x: IntRange, var y: IntRange, var z: IntRange, val on: Boolean) {
    fun clean() {
        this.x = max(x.first, -50)..min(x.last, 50)
        this.y = max(y.first, -50)..min(y.last, 50)
        this.z = max(z.first, -50)..min(z.last, 50)
    }

    private fun overlaps(c: Cuboid): Boolean {
        return overlaps1d(x, c.x) && overlaps1d(y, c.y) && overlaps1d(z, c.z)
    }

    private fun overlaps1d(r1: IntRange, r2: IntRange): Boolean {
        return r1.last >= r2.first
    }


    fun createSubCuboids(cuboids: List<Cuboid>) {
        val overlappingCuboids = cuboids.filter { c -> overlaps(c) }
    }

    fun intersect(other: Cuboid): Cuboid {
        if (overlaps(other)) {
            val x = max(x.first, other.x.first)..min(x.last, other.x.last)
            val y = max(y.first, other.y.first)..min(y.last, other.y.last)
            val z = max(z.first, other.z.first)..min(z.last, other.z.last)

            return Cuboid(x, y, z, true)
        }
        return Cuboid(0..0, 0..0, 0..0, true)
    }

    fun size(): Long {
        return abs(x.first - x.last).toLong() * abs(y.first - y.last).toLong() * abs(z.first - z.last).toLong()
    }

    override fun toString(): String {
        val prefix = if (on) "on " else "off "
        return prefix + x.first.toString() + ".." + x.last + "," + y.first + ".." + y.last + "," + z.first + ".." + z.last
    }
}